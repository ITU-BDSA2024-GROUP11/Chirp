...
public class ExampleTest : PageTest
{
    ...
    {
        await Page.GotoAsync($"{baseUrl}/");
        await Page.Locator("li").Filter(new() { HasText = "Jacqualine Gilcoine Save delete Starbuck now is what we hear the worst. — 2023-" }).GetByRole(AriaRole.Link).ClickAsync();
    ...
    public async Task End2endtest()
    {
        ...
        await Page.Locator("li").Filter(new() { HasText = "Jacqualine Gilcoine Follow Save Delete Starbuck now is what we hear the worst. — 2023-08-01" }).GetByRole(AriaRole.Link).ClickAsync();
